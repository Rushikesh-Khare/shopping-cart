...
async function createModel(req, res) {
    try {
        ...
        res.status(201).send({ status: true, data: creaOrderData, msg: "succesfully created" })
    } catch (error) {
...
async function updateModel(req, res) {
    try {
        ...
        if(!isValidObjectId(orderId)) {
            return res.status(400).send({ status: false, message: "invalid order object id" });
        }
        const findOrderId = await Orders.findById(orderId);
        if (!findOrderId) {
            return res.status(400).send({ status: false, message: "given order does not exist " });
